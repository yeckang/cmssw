...
GEMeMapping::GEMeMapping() : theVersion("") {}

GEMeMapping::GEMeMapping(const std::string& version) : theVersion(version) {}

GEMeMapping::~GEMeMapping() {}

const std::string& GEMeMapping::version() const { return theVersion; }

void GEMeMapping::setDummy() {
    ...
    for (int lphi = 0; lphi < maxVFat; ++lphi) {
      for (int ieta = 1; ieta <= maxiEtaId; ++ieta) {
        ...
        for (int i = 0; i < maxChan_; ++i) {
          ...
          GEMeMapping::channelNum cMap;
          cMap.chamberType = st;
          cMap.vfatAdd = chipPos;
          cMap.chNum = i;

          GEMeMapping::stripNum sMap;
          sMap.chamberType = st;
          if (st != 2) {
            sMap.iEta = ieta;
    ...
    for (int re = -1; re <= 1; re = re + 2) {
      ...
      for (int ch = 1; ch <= GEMDetId::maxChamberId; ++ch) {
        for (int ly = 1; ly <= maxLayerId; ++ly) {
          ...
          GEMeMapping::chamEC ec;
          ec.fedId = fedId;
          ec.gebId = gebId;
          ec.amcNum = amcNum;

          GEMeMapping::chamDC dc;
          dc.detId = gemId;
          dc.chamberType = st;
