...
public:
  ...
  GEMVFAT(const uint16_t BC, const uint32_t EC, const uint16_t chipID, const uint64_t lsDatas, const uint64_t msDatas);
  ~GEMVFAT() {}
  ...
  uint16_t bc() const { return VFATfirst{fw_}.bc; }
  uint8_t ec() const { return VFATfirst{fw_}.ec; }
  uint16_t vfatId() const { return VFATfirst{fw_}.pos; }
